package com.example.zq.play_android.data.source.local;

import android.database.Cursor;
import android.database.sqlite.SQLiteStatement;

import org.greenrobot.greendao.AbstractDao;
import org.greenrobot.greendao.Property;
import org.greenrobot.greendao.internal.DaoConfig;
import org.greenrobot.greendao.database.Database;
import org.greenrobot.greendao.database.DatabaseStatement;

import com.example.zq.play_android.bean.Carousel;

// THIS CODE IS GENERATED BY greenDAO, DO NOT EDIT.
/** 
 * DAO for table "CAROUSEL".
*/
public class CarouselDao extends AbstractDao<Carousel, Integer> {

    public static final String TABLENAME = "CAROUSEL";

    /**
     * Properties of entity Carousel.<br/>
     * Can be used for QueryBuilder and for referencing column names.
    */
    public static class Properties {
        public final static Property Desc = new Property(0, String.class, "desc", false, "DESC");
        public final static Property Id = new Property(1, int.class, "id", true, "ID");
        public final static Property ImagePath = new Property(2, String.class, "imagePath", false, "IMAGE_PATH");
        public final static Property IsVisible = new Property(3, int.class, "isVisible", false, "IS_VISIBLE");
        public final static Property Order = new Property(4, int.class, "order", false, "ORDER");
        public final static Property Title = new Property(5, String.class, "title", false, "TITLE");
        public final static Property Type = new Property(6, int.class, "type", false, "TYPE");
        public final static Property Url = new Property(7, String.class, "url", false, "URL");
    };


    public CarouselDao(DaoConfig config) {
        super(config);
    }
    
    public CarouselDao(DaoConfig config, DaoSession daoSession) {
        super(config, daoSession);
    }

    /** Creates the underlying database table. */
    public static void createTable(Database db, boolean ifNotExists) {
        String constraint = ifNotExists? "IF NOT EXISTS ": "";
        db.execSQL("CREATE TABLE " + constraint + "\"CAROUSEL\" (" + //
                "\"DESC\" TEXT," + // 0: desc
                "\"ID\" INTEGER PRIMARY KEY NOT NULL ," + // 1: id
                "\"IMAGE_PATH\" TEXT," + // 2: imagePath
                "\"IS_VISIBLE\" INTEGER NOT NULL ," + // 3: isVisible
                "\"ORDER\" INTEGER NOT NULL ," + // 4: order
                "\"TITLE\" TEXT," + // 5: title
                "\"TYPE\" INTEGER NOT NULL ," + // 6: type
                "\"URL\" TEXT);"); // 7: url
    }

    /** Drops the underlying database table. */
    public static void dropTable(Database db, boolean ifExists) {
        String sql = "DROP TABLE " + (ifExists ? "IF EXISTS " : "") + "\"CAROUSEL\"";
        db.execSQL(sql);
    }

    @Override
    protected final void bindValues(DatabaseStatement stmt, Carousel entity) {
        stmt.clearBindings();
 
        String desc = entity.getDesc();
        if (desc != null) {
            stmt.bindString(1, desc);
        }
        stmt.bindLong(2, entity.getId());
 
        String imagePath = entity.getImagePath();
        if (imagePath != null) {
            stmt.bindString(3, imagePath);
        }
        stmt.bindLong(4, entity.getIsVisible());
        stmt.bindLong(5, entity.getOrder());
 
        String title = entity.getTitle();
        if (title != null) {
            stmt.bindString(6, title);
        }
        stmt.bindLong(7, entity.getType());
 
        String url = entity.getUrl();
        if (url != null) {
            stmt.bindString(8, url);
        }
    }

    @Override
    protected final void bindValues(SQLiteStatement stmt, Carousel entity) {
        stmt.clearBindings();
 
        String desc = entity.getDesc();
        if (desc != null) {
            stmt.bindString(1, desc);
        }
        stmt.bindLong(2, entity.getId());
 
        String imagePath = entity.getImagePath();
        if (imagePath != null) {
            stmt.bindString(3, imagePath);
        }
        stmt.bindLong(4, entity.getIsVisible());
        stmt.bindLong(5, entity.getOrder());
 
        String title = entity.getTitle();
        if (title != null) {
            stmt.bindString(6, title);
        }
        stmt.bindLong(7, entity.getType());
 
        String url = entity.getUrl();
        if (url != null) {
            stmt.bindString(8, url);
        }
    }

    @Override
    public Integer readKey(Cursor cursor, int offset) {
        return cursor.getInt(offset + 1);
    }    

    @Override
    public Carousel readEntity(Cursor cursor, int offset) {
        Carousel entity = new Carousel( //
            cursor.isNull(offset + 0) ? null : cursor.getString(offset + 0), // desc
            cursor.getInt(offset + 1), // id
            cursor.isNull(offset + 2) ? null : cursor.getString(offset + 2), // imagePath
            cursor.getInt(offset + 3), // isVisible
            cursor.getInt(offset + 4), // order
            cursor.isNull(offset + 5) ? null : cursor.getString(offset + 5), // title
            cursor.getInt(offset + 6), // type
            cursor.isNull(offset + 7) ? null : cursor.getString(offset + 7) // url
        );
        return entity;
    }
     
    @Override
    public void readEntity(Cursor cursor, Carousel entity, int offset) {
        entity.setDesc(cursor.isNull(offset + 0) ? null : cursor.getString(offset + 0));
        entity.setId(cursor.getInt(offset + 1));
        entity.setImagePath(cursor.isNull(offset + 2) ? null : cursor.getString(offset + 2));
        entity.setIsVisible(cursor.getInt(offset + 3));
        entity.setOrder(cursor.getInt(offset + 4));
        entity.setTitle(cursor.isNull(offset + 5) ? null : cursor.getString(offset + 5));
        entity.setType(cursor.getInt(offset + 6));
        entity.setUrl(cursor.isNull(offset + 7) ? null : cursor.getString(offset + 7));
     }
    
    @Override
    protected final Integer updateKeyAfterInsert(Carousel entity, long rowId) {
        return entity.getId();
    }
    
    @Override
    public Integer getKey(Carousel entity) {
        if(entity != null) {
            return entity.getId();
        } else {
            return null;
        }
    }

    @Override
    protected final boolean isEntityUpdateable() {
        return true;
    }
    
}
